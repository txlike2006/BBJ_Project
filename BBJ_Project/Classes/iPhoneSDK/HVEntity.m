...
//  CircleRotate
//
//  Created by BJB041511-032 on 16/4/1.
...
+ (NSArray *)parseToEntityArray:(NSArray *)array withType:(Class)type
{
    ...
    [array enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if ([obj isKindOfClass:[NSDictionary class]] && [type isSubclassOfClass:[HVEntity class]]) {
            HVEntity *entity = [type entityWithDictionary:obj];
            if (entity) {
...
+ (NSArray *)parseToDictionaryArray:(NSArray *)array
{
    ...
    [array enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        if ([obj isKindOfClass:[HVEntity class]]) {
...
- (void)parseValueFromDic:(NSDictionary *)dict toClass:(Class)class parseArray:(BOOL)parseArray
{
    ...
    for (int i = 0; i < propertyCount; i++) {
        ...
        //字典（对象）
            ...
            else if ([NSClassFromString(classNameString) isSubclassOfClass:[HVEntity class]])
            {
...
- (void)parseValueToDic:(NSMutableDictionary *)dict fromClass:(Class)class
{
    ...
    for (int i = 0; i < propertyCount; i++) {
        ...
        if (val) {
            if ([val isKindOfClass:[HVEntity class]]) {
